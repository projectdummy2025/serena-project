...
async def supervisor_reason_node(user_prompt: str, user_id: int, retrieved_context: str) -> Dict[str, Any]:
    ...
    system_prompt = (
        ...
        "Anda berkomunikasi secara manusiawi, ramah, sopan, santun, dan profesional dengan pemilik/pendiri (pengguna).\n"
        f"Profil & Preferensi Personalisasi Pengguna:\n{user_profile}\n\n"
        "PERATURAN MUTLAK:\n"
        ...
        "3. DILARANG menggunakan tanda '>' di awal kalimat/paragraf.\n"
        f"Informasi Waktu Sistem: {now_str}.\n"
        f"Direktori Kerja: {config.WORKSPACE_DIR}.\n\n"
        f"Konteks Rujukan Second Brain (Obsidian Vault):\n{retrieved_context}\n\n"
        "Tugas Anda:\n"
        "1. Analisis pesan pengguna dengan mempertimbangkan riwayat percakapan & Konteks Rujukan Second Brain (Obsidian) di atas.\n"
        "2. Klasifikasikan intent secara akurat:\n"
        "   - 'CHAT': untuk percakapan/sapaan/pertanyaan tentang catatan Obsidian/tanya jawab umum.\n"
        "   - 'BRAINSTORMING': saat pengguna berdiskusi tentang ide baru, konsep proyek, perencanaan, atau manajemen kebiasaan/rutinitas. Serena berdiskusi interaktif, merangkum poin inti, dan DI AKHIR BALASAN SECARA PROAKTIF MENANYAKAN: 'Apakah poin ide / kebiasaan ini ingin saya catatkan ke folder 00-Inbox Obsidian Second Brain Anda?'\n"
        "   - 'SAVE_NOTE': saat pengguna membalas setuju untuk menyimpan ide/kebiasaan (misal: 'ya catatkan', 'boleh simpan', 'simpan ke obsidian'). Serena mengekstrak poin penting dan menyiapkan berkas catatan.\n"
        "   - 'TASK': khusus untuk perintah koding/eksekusi terminal/manipulasi berkas proyek lokal.\n"
        "   - 'RESEARCH': khusus untuk tugas riset web/pencarian berita luar.\n"
        "3. Berikan keluaran format JSON valid:\n"
        "{\n"
        '  "intent": "CHAT", "BRAINSTORMING", "SAVE_NOTE", "TASK", atau "RESEARCH",\n'
        '  "chat_response": "Jawaban ramah, terstruktur, & informatif jika CHAT/BRAINSTORMING/SAVE_NOTE (kosongkan jika TASK/RESEARCH)",\n'
        ...
        '  "note_title": "Judul singkat berkas catatan jika intent SAVE_NOTE, kosongkan jika tidak",\n'
        '  "note_content": "Poin-poin isi catatan rapi jika intent SAVE_NOTE, kosongkan jika tidak"\n'
        "}"
    )
        ...
        if intent == "SAVE_NOTE":
            ...
            try:
                ...
                if not chat_response:
                    chat_response = (
                        f"Catatan ide / kebiasaan telah berhasil saya simpan secara rapi ke Obsidian Second Brain Anda:\n\n"
                        f"- Judul Berkas : `{note_title}.md`\n"
                        f"- Lokasi : `00-Inbox/`\n\n"
                        f"Ringkasan Poin :\n{note_content}"
                    )
            except Exception as e:
                logger.warning(f"Kendala menyimpan catatan otomatis: {e}")
