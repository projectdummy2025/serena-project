...
async def supervisor_reason_node(user_prompt: str, user_id: int, retrieved_context: str) -> Dict[str, Any]:
    ...
    system_prompt = (
        ...
        "   - 'SYSTEM': khusus untuk inisialisasi proyek Git, penulisan berkas .env, eksekusi perintah Linux cepat, serta INSPEKSI KONDISI PERANGKAT / HOST (seperti cek RAM/CPU via free -h atau uptime, cek sisa kapasitas disk via df -h, periksa port aktif via ss -tuln, cek kontainer Docker via docker ps, dsb.). Tetapkan system_action = 'EXECUTE_COMMAND' dan tentukan linux_command yang tepat.\n"
        "   - 'TASK': khusus untuk rekayasa perangkat lunak mendalam yang membutuhkan Claude Code (analisis kode mendalam, bedah repositori aplikasi, pembuatan fitur baru, refactoring, perbaikan bug logika kompleks).\n"
        "   - 'RESEARCH': khusus untuk tugas riset web / pencarian dokumentasi eksternal.\n"
        "   - 'CHAT': untuk percakapan santai, sapaan, curhat, opini umum, atau bertukar pikiran santai. Jika pengguna mengajak santai (misal: 'santai dulu', 'ngobrol santai', 'capek nih'), tanggapi secara hangat, bersahabat, dan manusiawi layaknya partner bicara tanpa jargon kaku.\n"
        "   - 'BRAINSTORMING': saat pengguna berdiskusi tentang ide baru, konsep proyek, atau perencanaan konseptual tanpa perintah koding langsung.\n"
        "   - 'SAVE_NOTE': saat pengguna menyetujui atau meminta merangkum ide penting ke Obsidian Kotak Masuk.\n"
